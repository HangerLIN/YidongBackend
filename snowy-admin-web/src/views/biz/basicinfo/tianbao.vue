<template>
	<div id="table-wrap">
		<h2>{{props.column.subtemplateName}}表:</h2>
...
<!--					</div>-->
		...
		<h2></h2>
		<el-button @click="con2">保存</el-button>
		<el-button @click="count">计算</el-button>
		<el-button @click="transformDataList(props.subrecord)">显示</el-button>
...
const shourucyc = inject('shouruCycle');
...
const con = () =>{
	...
	console.log('qwwdkdkdkkdddjdj')
	console.log(props.cyc)
	console.log('qwwdkdkdkkdddjdj')
	console.log(foo.value)
	console.log('qwwdkdkdkkdddjdj')
	console.log(shourucyc.value)
	console.log('qwwdkdkdkkdddjdj')
	console.log(proId.value)
	console.log('qwwdkdkdkkdddjdj')
	console.log(props.subrecord)
	console.log('qwwdkdkdkkdddjdj')
	console.log(props.column.basicInformation)

	// tablename=props.column.
	const basicInformation = ref(props.column.basicInformation);
	const array = JSON.parse(basicInformation.value);
	console.log(array)
	const columnList1 = computed(() => {
	...
	columnList.value=columnList1.value
	console.log(columnList1.value)
	console.log(columnList.value)
	addYearColumns()
}
onMounted(con);
...
const con2 = () =>{
	...
	questionChoiceVOlist.value.forEach(obj => {
		...
		Object.keys(obj).forEach(key => {
			...
			if (typeof key === 'string') {
				if (key.indexOf('20') !== -1) {
					if (key.indexOf('2024') !== -1){
						yearinfo.push(obj[key]);
						console.log(yearinfo)
					}else {
						yearinfo1.push(obj[key]);
						console.log(yearinfo1)
					}// 使用indexOf作为替代方案
					// 如果是，将键值对作为一个对象添加到basicInformation数组中

				}else if(key.indexOf('row') == -1){ // 使用indexOf作为替代方案
					// 如果是，将键值对作为一个对象添加到basicInformation数组中
					basicInformation.push(obj[key]);
				}
			} else {
				// 如果key不是字符串，可以在这里处理或记录错误
				console.error('Key is not a string:', key);
		...
		// 将basicInformation数组添加到resultArray中
		resultArray.push({basicInformation: basicInformation,startYearData: yearinfo1[0] ,endYearData: yearinfo });
	});
	console.log(resultArray)
	console.log(questionChoiceVOlist.value)
	console.log(newArray)
...
// 输出将包含每个项目的成本，按年份组织
	...
	dataform = {projectId:proId.value,templateId:props.tempID,subtemplateId:props.subrecord,data:resultArray}
//		resultArray.push({ projectId:proId.value, basicInformation: basicInformation,startYearData: yearinfo1 ,endYearData: yearinfo ,subtemplateId:props.subrecord,templateId:props.tempID});
...
const response=ref()
const count = () => {
	dataform = {ProjectId:proId.value,TemplateId:props.tempID}
	projectBasicInfoApi
		.calculate2({
	...
	// const transformedData = transformDataList(44);
...
// let itemList =ref([])】
const x = () => {
	suss.value=props.subrecord
}
onMounted(x)
function transformDataList(id){
	console.log('DataListsssssss')
	const total = JSON.parse(JSON.stringify(response.value))
	console.log(total)
	const DataList1=total.submouleDVList
	console.log(props.subrecord)
	const numericSubrecord = parseInt(id, 10)
	console.log('Type of props.subrecord:', typeof numericSubrecord)
	console.log(DataList1)
	let itemList = null; // 初始化 itemList 为 null，表示尚未找到匹配项
// 假设 DataList1 是已经定义且包含元素的数
	for (let i = 0; i < DataList1.length; i++) {
		const item = DataList1[i]; // 获取当前遍历到的元素
		if (item.subtemplateId === numericSubrecord) { // 检查是否满足条件
			itemList = item; // 如果满足条件，则将当前元素赋值给 itemList
			break; // 找到匹配项后退出循环
		}
	}
	...
	let baseYear = 2024
	const transformedData = itemList.data.map((item, index) => {
		const result = {};
		result[baseYear.toString()] = item.startYearData;
		const years = item.endYearData.reduce((acc, endYearValue, index) => {
			const year = baseYear + index + 1;

			// 将年份和对应的值添加到结果对
			acc[year.toString()] = endYearValue;
			// 返回累积的结果对象（实际上是数组，但在这里我们用它来模拟对象的累积）
			return acc;
		}, {}); // 初始值为一个空对象
		const aa=Object.assign(result, years)
		// 将reduce生成的对象合并到result中（实际上在这个例子中，result只包含startYearData，但我们直接修改了acc）
		Object.assign(state.questionChoiceVOlist[index], aa);
		console.log(state.questionChoiceVOlist)
		console.log(aa)
		return aa;
...
// 调用函数
// const transformedData = processData(DataList, 34);
// console.log(transformedData);
